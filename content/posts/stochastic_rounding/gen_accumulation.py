...
def make_plot(t):
    ...
    ax.plot(steps, fp32, color=t.gray, linewidth=2.5, label='fp32', zorder=3)
    ax.plot(steps, rne, color=t.red, linewidth=2.5, label='round-to-nearest', zorder=3)
    ax.fill_between(steps, sr_lo, sr_hi, color=t.blue, alpha=0.15, zorder=1)
    ax.plot(steps, sr_mean, color=t.blue, linewidth=2, linestyle='--',
            label='stochastic rounding (mean + 90% CI)', zorder=2)
    ax.set_xlabel('Step', fontsize=12)
    ax.set_ylabel('Accumulated value', fontsize=12)
    ax.legend(fontsize=9, loc='upper left', framealpha=t.legend_alpha,
              edgecolor=t.legend_edge, fancybox=False)
    ax.grid(True, which='major', alpha=0.3)
    fig.tight_layout()
    return fig


savefig(make_plot, 'accumulation.png', dpi=180)
